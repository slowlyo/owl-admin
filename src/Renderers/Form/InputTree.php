 ...
 * @method self searchable($value = true) 是否可检索，仅在 type 为 tree-select 的时候生效
 * @method self hideRoot($value) 如果想要显示个顶级节点，请设置为 false
 * @method self rootLabel($value) 当 hideRoot 不为 false 时有用，用来设置顶级节点的文字。
 * @method self showIcon($value = true) 是否显示图标
 * @method self showRadio($value = true) 是否显示单选按钮，multiple 为 false 是有效。
 * @method self showOutline($value = true) 是否显示树层级展开线
 * @method self initiallyOpen($value = true) 设置是否默认展开所有层级。
 ...
 * @method self rootCreatable($value = true) 是否可以创建顶级节点
 ...
 * @method self enableNodePath($value = true) 是否开启节点路径模式
