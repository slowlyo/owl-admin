 ...
 * @method self embed($value = true) 内嵌展示
 * @method self title($value) 弹窗配置的顶部标题
 * @method self fields($value) 字段配置
 * @method self showANDOR($value = true) 用于 simple 模式下显示切换按钮
 * @method self showNot($value = true) 是否显示「非」按钮
 * @method self searchable($value = true) 字段是否可搜索
 *
 */
