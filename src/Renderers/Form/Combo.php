 ...
 * @method self flat($value = true) 是否将结果扁平化(去掉 name), 只有当 items 的 length 为 1 且 multiple 为 true 的时候才有效。
 * @method self joinValues($value = true) 默认为 true 当扁平化开启的时候，是否用分隔符的形式发送给后端，否则采用 array 的方式。
 ...
 * @method self draggableTip($value = true) 可拖拽的提示文字
