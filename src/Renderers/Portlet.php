 ...
 * @method self hideHeader($value = true) 隐藏头部
 * @method self divider($value) 去掉分隔线
 * @method self mountOnEnter($value = true) 只有在点中 tab 的时候才渲染
 * @method self unmountOnExit($value = true) 切换 tab 的时候销毁
 * @method self scrollable($value = true) 是否导航支持内容溢出滚动，vertical和chrome模式下不支持该属性；chrome模式默认压缩标签
 */
