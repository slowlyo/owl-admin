 ...
 * @method self showCloseButton($value = true) 是否展示关闭按钮，当值为 false 时，默认开启 closeOnOutside
 * @method self closeOnEsc($value = true) 是否支持按 Esc 关闭 Drawer
 * @method self closeOnOutside($value = true) 点击内容区外是否关闭 Drawer
 * @method self overlay($value = true) 是否显示蒙层
 * @method self resizable($value = true) 是否可通过拖拽改变 Drawer 大小
