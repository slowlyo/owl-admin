...
export const useRouteStore = defineStore('route-store', {
	...
	actions: {
		...
		async initDynamicRoute() {
			...
			// @ts-ignore
			const {status, data} = await fetchUserRoutes();

			if (!status) {
				// @ts-ignore
				this.handleAuthRoute(data);
				// @ts-ignore
				data?.forEach((item: any) => {
					if (item?.is_home) {
						this.routeHomeName = item.name;
						this.handleUpdateRootRedirect(item.path.replace('/', ''));

						initHomeTab(item.name, router);
					}
				})
